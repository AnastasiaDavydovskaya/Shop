...
public class OrderMvcTest {
    ...
    @Test
    @WithMockUser
    void testGetListOrders() throws Exception {
        when(orderService.findAll()).thenReturn(List.of());

        mvc.perform(get("/list/orders")
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(view().name("list-orders"));
    }

    @Test
    @WithMockUser
    void testDeleteOrder() throws Exception {
        doThrow(new ResourceNotFoundException("Not found")).when(orderService).deleteById(1L);
